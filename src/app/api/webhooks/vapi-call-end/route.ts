/**
 * Ziarem Enterprise: Vapi call-end webhook.
 * POST /api/webhooks/vapi-call-end
 * Body: transcript (required), recordingUrl/recording_url optional, phone_number/phone optional.
 * Pipeline: Gemini extraction → Supabase upsert (leads, calls) → child leads + cross_sells → n8n.
 */
...
import { uploadRecordingToStorage } from "@/lib/storage/uploadRecording";
...
export async function POST(request: Request) {
  try {
    ...
    type LeadRow = { id: string; phone_number: string; first_name: string | null; last_name: string | null; preferred_language: string; location: string; estimated_value: number | null };
    let leadRow: LeadRow;
    if (existing) {
    ...
    const vapiRecordingUrl = getRecordingUrl(body);
    let recordingUrl: string | null = vapiRecordingUrl;
    if (vapiRecordingUrl) {
      const storageUrl = await uploadRecordingToStorage(
        vapiRecordingUrl,
        crypto.randomUUID()
      );
    ...
    await supabaseAdmin.from("interactions").insert({
      ...
      lead_id: leadId,
      phone_number: phone,
      summary: extracted.lead_intent ?? null,
      transcript,
      payload: {
        call_id: callRow.id,
        company_id: companyId,
    ...
    console.error("[vapi-call-end webhook]", e);
    return NextResponse.json(
